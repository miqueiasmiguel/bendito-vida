...
A tela "Meus Mixes" (`/(tabs)/profile/my-mixes`) SHALL buscar e exibir os mixes salvos via `useProfileStore`, renderizando cada item com `SavedMixCard`. O botão de voltar do header SHALL retornar para o hub de Perfil. Cada `SavedMixCard` SHALL ser tapeável e, ao ser pressionado, SHALL navegar para `/(tabs)/profile/mix-detail?mixId=<id>` exibindo os detalhes do mix selecionado.
...
#### Scenario: Toque em um SavedMixCard
- **WHEN** o usuário toca em qualquer `SavedMixCard` da lista
- **THEN** o sistema navega para `/(tabs)/profile/mix-detail?mixId=<id>` com o `id` do mix correspondente
