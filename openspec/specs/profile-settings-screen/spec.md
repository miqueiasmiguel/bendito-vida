...
- **THEN** o sistema chama `updateName(userId, name)`, chama `useAuthStore.setUser({ ...user, name })` para refletir o novo nome imediatamente em toda a app (incluindo a saudação da Home), exibe feedback visual de sucesso ("Nome atualizado com sucesso!") e atualiza o estado local do store
...
O `useProfileStore` SHALL expor `updateName(userId: string, name: string): Promise<void>` que executa `supabase.from('profiles').update({ name }).eq('id', userId)` e atualiza `profile.name` no estado local em caso de sucesso. A sincronização com `useAuthStore.user.name` é responsabilidade do chamador (settings.tsx), que deve chamar `setUser` logo após o `updateName` resolver com sucesso.
