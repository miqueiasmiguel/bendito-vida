...
Cada rota sem implementação real (`(auth)/login`, `(auth)/register`, `(tabs)/home`, `(tabs)/simulator`, `(tabs)/progress`, `(tabs)/profile`, `result`) SHALL ter um arquivo `.tsx` mínimo que renderize ao menos um `<View>` com texto identificando a tela. A rota `(onboarding)/quiz` SHALL ter a implementação real do quiz (não placeholder).
