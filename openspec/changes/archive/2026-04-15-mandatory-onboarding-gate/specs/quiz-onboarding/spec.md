...
Ao responder a última pergunta e confirmar, o sistema SHALL: (1) chamar `markOnboardingComplete()` do `useAuthStore` para persistir a flag no Supabase, (2) chamar `useQuizStore.setProfile(profile)` para salvar o perfil calculado, e (3) navegar para `/(tabs)/home` usando `router.replace` (não `router.push`), para que o stack de navegação não permita voltar ao quiz após a conclusão. A persistência da flag SHALL ocorrer antes da navegação.
