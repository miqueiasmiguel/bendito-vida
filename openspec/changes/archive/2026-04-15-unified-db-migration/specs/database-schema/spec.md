...
A migration SHALL criar a tabela `weekly_checkins` com as colunas: `id uuid PRIMARY KEY DEFAULT gen_random_uuid()`, `user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE`, `week text NOT NULL` (formato `YYYY-WNN`, ex: `"2026-W15"`), `energy_score smallint NOT NULL CHECK (energy_score BETWEEN 1 AND 5)`, `sleep_score smallint NOT NULL CHECK (sleep_score BETWEEN 1 AND 5)`, `focus_score smallint NOT NULL CHECK (focus_score BETWEEN 1 AND 5)`, `created_at timestamptz NOT NULL DEFAULT now()`. A constraint `UNIQUE(user_id, week)` SHALL garantir um único check-in por semana por usuário. A tabela SHALL ter RLS com policies de SELECT e INSERT restritas ao `auth.uid() = user_id`.

#### Scenario: Tabela weekly_checkins criada com RLS
- **WHEN** a migration é aplicada
- **THEN** a tabela `weekly_checkins` existe com as 7 colunas, constraint unique em `(user_id, week)`, checks de range nos scores, RLS ativo e as 2 policies presentes

#### Scenario: Constraint de unicidade por semana
- **WHEN** um INSERT tenta adicionar um segundo check-in com o mesmo `user_id` e `week`
