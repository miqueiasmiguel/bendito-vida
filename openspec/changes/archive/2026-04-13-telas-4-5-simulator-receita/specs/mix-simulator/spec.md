...
A lista de ingredientes SHALL exibir todos os ingredientes de `src/data/ingredients.ts` como IngredientCards em um `ScrollView` horizontal. Cada card SHALL exibir: nome do ingrediente, círculo colorido (cor do ingrediente), caloria em kcal, e badge "Paraibano" em `primary-700` se `isParaibano === true`. O card SHALL ter borda `primary-700` quando o ingrediente estiver selecionado no mix. Os cards SHALL ter altura fixa (não esticar para preencher o container) — garantido por `alignSelf: 'flex-start'` no estilo do card.

#### Scenario: Ingrediente não paraibano
- **WHEN** o ingrediente tem `isParaibano: false`
- **THEN** o card é exibido sem o badge "Paraibano"

#### Scenario: Ingrediente paraibano
- **WHEN** o ingrediente tem `isParaibano: true`
- **THEN** o card exibe badge "Paraibano" com fundo `primary-100` e texto `primary-700`

#### Scenario: Ingrediente selecionado
- **WHEN** o ingrediente está na lista `selectedIngredients` do store
- **THEN** o card exibe borda `primary-700` de 2px e fundo `primary-100`

#### Scenario: Cards com altura natural
- **WHEN** os cards são renderizados no ScrollView horizontal
- **THEN** cada card exibe apenas sua altura de conteúdo, sem esticar verticalmente para preencher o ScrollView
