...
Cada rota sem implementação real (`(auth)/login`, `(auth)/register`, `(tabs)/progress`, `(tabs)/profile`) SHALL ter um arquivo `.tsx` mínimo que renderize ao menos um `<View>` com texto identificando a tela. As rotas `(onboarding)/quiz`, `(tabs)/home`, `(tabs)/simulator` e `result` SHALL ter implementação real (não placeholder). A rota `result` SHALL ser acessível como stack screen no root layout.
